...
import pekko.projection.grpc.consumer.ConsumerFilter.FilterCriteria
...
@InternalApi private[pekko] object ConsumerFilterStore {
  ...
  def useDistributedData(system: ActorSystem[_]): Boolean = {
    system.classicSystem
      ...
      .provider
      .getClass
      .getName == "org.apache.pekko.cluster.ClusterActorRefProvider" &&
    system.dynamicAccess.getClassFor[Any]("org.apache.pekko.cluster.ddata.typed.scaladsl.DistributedData").isSuccess
  }
  ...
  def createDdataConsumerFilterStore(
      ...
      streamId: String,
      notifyUpdatesTo: ActorRef[ConsumerFilterRegistry.FilterUpdated]): Behavior[Command] = {
    val className = "org.apache.pekko.projection.grpc.internal.DdataConsumerFilterStore"
    system.classicSystem
      .asInstanceOf[ExtendedActorSystem]
      .dynamicAccess
 ...
 * INTERNAL API
 */
@InternalApi private[pekko] class LocalConsumerFilterStore(
 ...
 * INTERNAL API
 */
@InternalApi private[pekko] object DdataConsumerFilterStore {
  object State {
 ...
 * INTERNAL API
 */
@InternalApi private[pekko] class DdataConsumerFilterStore(
    context: ActorContext[ConsumerFilterStore.Command],
    settings: ConsumerFilterSettings,
