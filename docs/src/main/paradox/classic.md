...
The `ActorSystem` is a parameter in several places of the Projections API. That is the `pekko.actor.typed.ActorSystem`.
Given a classic `pekko.actor.ActorSystem` it can be adapted to an `pekko.actor.typed.ActorSystem` like this:
...
them in `pekko.persistence.journal.Tagged`, which can be done in the `PersistentActor` or by using
...
`ShardedDaemonProcess` can be used in the same way with a classic `pekko.actor.ActorSystem`, after adapting it to
`pekko.actor.typed.ActorSystem` as described @ref:[above](#actor-system).
