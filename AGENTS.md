...
- Projection handler changes must preserve at-least-once or exactly-once semantics as documented.
...
- Projection tests must verify offset management behavior (at-least-once, exactly-once).
...
- Projection handlers must be idempotent where at-least-once semantics are used.
- Backend-specific implementations (JDBC, R2DBC, Cassandra, Kafka) must follow the SPI pattern.
